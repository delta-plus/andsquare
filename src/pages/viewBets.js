...
export default class viewBets extends Component {
  ...
  async setValues() {
    ...
    const userDB = Firebase.database().ref(emailHash);
  ...
  async accept(user, position, wager, ID, key) {
    // Push to both databases using key to get proper node.
    const userEmail = await AsyncStorage.getItem('user_email');
    const emailHash = makeHash(userEmail).toString().replace("-","_");
    const emailHash2 = makeHash(user).toString().replace("-","_");
    const confirmedBetsDB = Firebase.database().ref(emailHash + 2); 
    const confirmedBetsDB2 = Firebase.database().ref(emailHash2 + 2);
    const today = new Date().toDateString();

    confirmedBetsDB.push().set({
      date: today,
      position: position,
      wager: wager,
      user: user,
            ...
            <View style={{marginBottom: 15}}>
              ...
              { Boolean(this.state.check == rowData.set.ID) &&
                <View style={{flexDirection: 'row'}}>
                  <Button onpress = {this.accept.bind(this, 
                                                      rowData.set.user, 
                                                      rowData.set.position, 
                                                      rowData.set.wager, 
                                                      rowData.set.ID, 
  ...
  render(){
    return (
      <View style={styles.container}>
        <Header text="Bets Offered" loaded={this.state.loaded} />  
